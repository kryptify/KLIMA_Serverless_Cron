...
In order to set the wallet private key and swap percent, you should set them in AWS CLI.

```
aws ssm put-parameter --name "/klima/private-key" --value "the wallet private key" --type "SecureString" --overwrite

aws ssm put-parameter --name "/klima/swap-percent" --value "percent of the claimed KLIMA for swapping" --type "String" --overwrite
```

### Testing with Mocha

In order to do unit test, you can do as following.
